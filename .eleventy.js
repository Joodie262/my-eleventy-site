...
module.exports = async function (eleventyConfig) {
  ...
  // Disable HTML <base> auto-prefixing so we can use | url everywhere
  const { HtmlBasePlugin } = await import("@11ty/eleventy");
  eleventyConfig.addPlugin(HtmlBasePlugin, {
    // Apply to which output extensions. Use false to opt-out entirely.
    extensions: false,
    // baseHref would default to pathPrefix, but we’re opting out anyway.
  });
  ...
  function renderIcon(name, opts = {}) {
    ...
    const {
      ...
      style,               // allow inline style pass-through
    } = opts;
    ...
    svg = svg.replace(/<svg\b([^>]*)>/, (m, attrs) => {
      ...
      if (style) {
        if (/\bstyle="/.test(a)) {
          a = a.replace(/\bstyle="([^"]*)"/, (_, s) => `style="${s} ${style}"`);
        } else {
          a += ` style="${style}"`;
        }
      }
  ...
  eleventyConfig.addGlobalData("eleventyComputed", {
    ...
    // Do not define `date` here—handled by src/*.11tydata.js
  });
  ...
  console.log("[11ty] pathPrefix:", pathPrefix, "| outDir:", outDir);
